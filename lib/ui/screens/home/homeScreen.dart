...
import 'package:nunyaexam/features/profileManagement/cubits/updateScoreAndCoinsCubit.dart';
import 'package:nunyaexam/features/profileManagement/cubits/updateUserDetailsCubit.dart';
import 'package:nunyaexam/features/profileManagement/profileManagementLocalDataSource.dart';
...
import 'package:nunyaexam/ui/screens/battle/widgets/randomOrPlayFrdDialog.dart';
import 'package:nunyaexam/ui/screens/battle/widgets/roomDialog.dart';
import 'package:nunyaexam/ui/screens/home/widgets/appUnderMaintenanceDialog.dart';
import 'package:nunyaexam/ui/screens/home/widgets/menuBottomSheetContainer.dart';
import 'package:nunyaexam/ui/screens/profile/widgets/editProfileFieldBottomSheetContainer.dart';
...
import 'package:nunyaexam/features/auth/cubits/referAndEarnCubit.dart';
import 'package:nunyaexam/features/profileManagement/cubits/userDetailsCubit.dart';
import 'package:nunyaexam/features/profileManagement/models/userProfile.dart';
...
import 'package:nunyaexam/ui/screens/home/widgets/quizTypeContainer.dart';
import 'package:nunyaexam/ui/screens/home/widgets/updateAppContainer.dart';
...
import 'package:nunyaexam/ui/widgets/userAchievementScreen.dart';
import 'package:nunyaexam/utils/errorMessageKeys.dart';
import 'package:nunyaexam/utils/quizTypes.dart';
...
class _HomeScreenState extends State<HomeScreen>
  ...
  void setQuizMenu() {
    Future.delayed(Duration.zero, () {
      ...
      if (!systemCubit.isMathQuizAvailable()) {
        _quizTypes
            .removeWhere((element) => element.quizTypeEnum == QuizTypes.bece);
      }

      // WASSCE
      if (!systemCubit.isMathQuizAvailable()) {
        _quizTypes
            .removeWhere((element) => element.quizTypeEnum == QuizTypes.wassce);
      }

      // CAMBRIDGE O LEVEL
      if (!systemCubit.isMathQuizAvailable()) {
        _quizTypes.removeWhere(
            (element) => element.quizTypeEnum == QuizTypes.cambridgeOLevel);
      }

      // CAMBRIDGE A LEVEL
      if (!systemCubit.isMathQuizAvailable()) {
        _quizTypes.removeWhere(
            (element) => element.quizTypeEnum == QuizTypes.cambridgeALevel);
      }
      // TOFEL
      if (!systemCubit.isMathQuizAvailable()) {
        _quizTypes
            .removeWhere((element) => element.quizTypeEnum == QuizTypes.tofel);
      }

      // SAT
      if (!systemCubit.isMathQuizAvailable()) {
        _quizTypes
            .removeWhere((element) => element.quizTypeEnum == QuizTypes.sat);
      }

      // SAT
      if (!systemCubit.isMathQuizAvailable()) {
        _quizTypes
            .removeWhere((element) => element.quizTypeEnum == QuizTypes.ielts);
      }
  ...
  Future<void> generateImageNotification(String title, String msg, String image,
    ...
    var androidPlatformChannelSpecifics = AndroidNotificationDetails(
      'com.epahubb.nunyaexam', //channel id
      'nunyaexam', //channel name
      channelDescription: 'nunyaexam',
...
// QUIZ TYPES
  void _onQuizTypeContainerTap(int quizTypeIndex) {
    // IF THE QUIZ TYPE SELECTED IS DAILY QUIZ
    if (_quizTypes[quizTypeIndex].quizTypeEnum == QuizTypes.dailyQuiz) {
      //  CHECK FROM THE ADMIN IF THE DAILY QUIZ MODULE IS ACTIVATED
      if (context.read<SystemConfigCubit>().getIsDailyQuizAvailable() == "12") {
      ...
      // ELSE IF THE QUIZ TYPE IS TRUE OR FALSE REDIRECT TO TRUE OR FALSE
      Navigator.of(context).pushNamed(Routes.quiz, arguments: {
      ...
      // CONTEST
      if (context.read<SystemConfigCubit>().getIsContestAvailable() == "12") {
    ...
    // BECE
    else if (_quizTypes[quizTypeIndex].quizTypeEnum == QuizTypes.bece) {
      ...
      Navigator.of(context)
          .pushNamed(Routes.category, arguments: {"quizType": QuizTypes.bece});
    }
    // WASSCE REDIRECT
    else if (_quizTypes[quizTypeIndex].quizTypeEnum == QuizTypes.wassce) {
      Navigator.of(context).pushNamed(Routes.category,
          arguments: {"quizType": QuizTypes.wassce});
    }
    // CAMBRIDGE O LEVEL REDIRECT
    else if (_quizTypes[quizTypeIndex].quizTypeEnum ==
      ...
      Navigator.of(context).pushNamed(Routes.category,
          arguments: {"quizType": QuizTypes.cambridgeOLevel});
    }
    // CAMBRIDGE A LEVEL REDIRECT
    else if (_quizTypes[quizTypeIndex].quizTypeEnum ==
        QuizTypes.cambridgeALevel) {
      Navigator.of(context).pushNamed(Routes.category,
          arguments: {"quizType": QuizTypes.cambridgeALevel});
    }
    // TOFEL REDIRECT
    else if (_quizTypes[quizTypeIndex].quizTypeEnum == QuizTypes.tofel) {
      Navigator.of(context)
          .pushNamed(Routes.category, arguments: {"quizType": QuizTypes.tofel});
    }
    // SAT REDIRECT
    else if (_quizTypes[quizTypeIndex].quizTypeEnum == QuizTypes.sat) {
      Navigator.of(context)
          .pushNamed(Routes.category, arguments: {"quizType": QuizTypes.sat});
    }
    // SAT REDIRECT
    else if (_quizTypes[quizTypeIndex].quizTypeEnum == QuizTypes.ielts) {
      Navigator.of(context).pushNamed(Routes.category,
