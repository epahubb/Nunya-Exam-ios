...
import 'package:nunyaexam/features/quiz/models/leaderBoardMonthly.dart';
import 'package:nunyaexam/features/quiz/quizException.dart';
import 'package:nunyaexam/features/quiz/quizRemoteDataSoure.dart';
    ...
    try {
      ...
      if (quizType == QuizTypes.dailyQuiz) {
        ...
        questions = result
            .map((question) => Question.fromJson(Map.from(question)))
            .toList();
      } else if (quizType == QuizTypes.bece) {
        String type = categoryId!.isNotEmpty ? "category" : "subcategory";
        String id = type == "category" ? categoryId : subcategoryId!;
        result =
            await _quizRemoteDataSource.getBeceQuestions(type: type, id: id);
        questions = result
            .map((question) => Question.fromJson(Map.from(question)))
            .toList();
      }
      // WASSCE O LEVEL QUESTIONS
      else if (quizType == QuizTypes.wassce) {
        String type = categoryId!.isNotEmpty ? "category" : "subcategory";
        String id = type == "category" ? categoryId : subcategoryId!;
        result =
            await _quizRemoteDataSource.getWassceQuestions(type: type, id: id);
        questions = result
            .map((question) => Question.fromJson(Map.from(question)))
            .toList();
      }
      // CAMBRIDGE O LEVEL QUESTIONS
      else if (quizType == QuizTypes.cambridgeOLevel) {
        String type = categoryId!.isNotEmpty ? "category" : "subcategory";
        String id = type == "category" ? categoryId : subcategoryId!;
        result = await _quizRemoteDataSource.getCambridgeOLevelQuestions(
            type: type, id: id);
        questions = result
            .map((question) => Question.fromJson(Map.from(question)))
            .toList();
      ...
      // CAMBRIDGE A LEVEL QUESTIONS
      else if (quizType == QuizTypes.cambridgeALevel) {
        String type = categoryId!.isNotEmpty ? "category" : "subcategory";
        String id = type == "category" ? categoryId : subcategoryId!;
        result = await _quizRemoteDataSource.getCambridgeALevelQuestions(
            type: type, id: id);
        questions = result
            .map((question) => Question.fromJson(Map.from(question)))
            .toList();
      ...
      // TOFEL QUESTIONS
      else if (quizType == QuizTypes.tofel) {
        String type = categoryId!.isNotEmpty ? "category" : "subcategory";
        String id = type == "category" ? categoryId : subcategoryId!;
        result =
            await _quizRemoteDataSource.getTofelQuestions(type: type, id: id);
        questions = result
            .map((question) => Question.fromJson(Map.from(question)))
            .toList();
      }
      ...
      // SAT QUESTIONS
      else if (quizType == QuizTypes.sat) {
        String type = categoryId!.isNotEmpty ? "category" : "subcategory";
        String id = type == "category" ? categoryId : subcategoryId!;
        result =
            await _quizRemoteDataSource.getSatQuestions(type: type, id: id);
        questions = result
            .map((question) => Question.fromJson(Map.from(question)))
            .toList();
      }
