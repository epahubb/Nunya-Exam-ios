...
class TournamentBattleStarted extends TournamentBattleState {
  ...
  TournamentBattleStarted(
      {required this.tournamentBattle,
...
class TournamentBattleCubit extends Cubit<TournamentBattleState> {
  ...
  TournamentBattleCubit(this._tournamentRepository)
      : super(TournamentBattleInitial());

  StreamSubscription<DocumentSnapshot>? _tournamentBattleSubscription;

  void _subscribeTournamentBattle(
      {required String tournamentBattleId, required String uid}) {
    _tournamentBattleSubscription = _tournamentRepository
        .listenToTournamentBattleUpdates(tournamentBattleId)
        .listen((event) {
      if (event.exists) {
        TournamentBattle tournamentBattle =
            TournamentBattle.fromDocumentSnapshot(event);
        ...
        if (tournamentBattle.battleType == TournamentBattleType.quaterFinal) {
          ...
          if (!tournamentBattle.readyToPlay) {
            ...
            emit(TournamentBattleStarted(
              ...
              questions: state is TournamentBattleStarted
                  ? (state as TournamentBattleStarted).questions
                  : tournamentBattle.questions,
            ));
          }
        } else if (tournamentBattle.battleType ==
              ...
              if (tournamentBattle.createdBy != uid) {
                ...
                if (tournamentBattle.createdBy == uid) {
                  ...
                  _tournamentRepository
                      .startTournamentBattle(tournamentBattleId);
                }
              }
            } else {
  ...
  void createTournamentBattle({
    ...
    _tournamentRepository
        .createTournamentBattle(
            tournamentBattleType: tournamentBattleType,
            tournamentId: tournamentId,
            user1: user1,
            user2: user2)
        .then((tournamentBattleId) {
      //add quater final details
      if (tournamentBattleType == TournamentBattleType.quaterFinal) {
        //update quater final details
        _tournamentRepository.addQuaterFinalDetails(
            tournamentId: tournamentId,
            quaterFinalBattleId: tournamentBattleId,
            user1Uid: user1.uid,
            user2Uid: user2.uid);
      }

      _subscribeTournamentBattle(
          tournamentBattleId: tournamentBattleId, uid: user1.uid);
    }).catchError((e) {
  ...
  void submitAnswer(String currentUserId, String submittedAnswer,
    ...
    if (state is TournamentBattleStarted) {
      TournamentBattle tournamentBattle =
          (state as TournamentBattleStarted).tournamentBattle;
      ...
      if (currentUserId == tournamentBattle.user1.uid) {
        if (tournamentBattle.user1.answers.length != questions.length) {
          _tournamentRepository.submitAnswer(
            ...
            points: isCorrectAnswer
                ? (tournamentBattle.user1.points + points)
                : tournamentBattle.user1.points,
            forUser1: true,
            submittedAnswer: List.from(tournamentBattle.user1.answers)
              ..add(submittedAnswer),
          );
        }
      } else {
        ...
        if (tournamentBattle.user2.answers.length != questions.length) {
          _tournamentRepository.submitAnswer(
            submittedAnswer: List.from(tournamentBattle.user2.answers)
              ..add(submittedAnswer),
            tournamentBattleId: tournamentBattle.tournamentBattleId,
            points: isCorrectAnswer
                ? (tournamentBattle.user2.points + points)
                : tournamentBattle.user2.points,
            forUser1: false,
          );
        }
      }
  ...
  void deleteRoom() {
    if (state is TournamentBattleStarted) {
      ...
      _tournamentRepository.removeTournamentBattle(
        tournamentBattleId: (state as TournamentBattleStarted)
            .tournamentBattle
            .tournamentBattleId,
      );
    }
  ...
  bool opponentLeftTheGame(String userId) {
    if (state is TournamentBattleStarted) {
      ...
      return (state as TournamentBattleStarted).hasLeft &&
          getCurrentUserDetails(userId).answers.length !=
              (state as TournamentBattleStarted).questions.length;
    }
    return false;
  }
  ...
  String getRoomId() {
    if (state is TournamentBattleStarted) {
      return (state as TournamentBattleStarted)
          .tournamentBattle
          .tournamentBattleId;
    }
  ...
  TournamentPlayerDetails getCurrentUserDetails(String currentUserId) {
    if (state is TournamentBattleStarted) {
      if (currentUserId ==
          (state as TournamentBattleStarted).tournamentBattle.user1.uid) {
  ...
  void _joinSemiFinal(
    ...
    try {
      //add semi final details
      final searchAgain = await _tournamentRepository.joinTournamentBattle(
          tournamentBattleId: tournamentBattleId,
          tournamentPlayerDetails: tournamentPlayerDetails);
      if (searchAgain) {
        searchSemiFinal(
