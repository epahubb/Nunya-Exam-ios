...
class TournamentCubit extends Cubit<TournamentState> {
  ...
  void _subscribeTournament(
      {required String tournamentId, required String uid}) {
    _tournamentSubscription = _tournamentRepository
        .listenToTournamentUpdates(tournamentId)
        .listen((event) {
      if (event.exists) {
  ...
  Future<void> serachTournament({
    ...
    try {
      List<Tournament> tournaments =
          await _tournamentRepository.searchTournament(
              questionLanguageId: languageId, title: tournamentTitle);
      ...
      if (tournaments.isEmpty) {
        ...
        List<int> totalPlayers =
            tournaments.map((e) => e.totalPlayers).toSet().toList();
        ...
        String tournamentId = tournaments
            .where((element) => element.totalPlayers == totalPlayers.first)
            .toList()
            .first
            .id;
        _joinTournament(
  ...
  void _createTournament({
    ...
    try {
      String tournamentId = await _tournamentRepository.createTournament(
          name: name,
          uid: uid,
          profileUrl: profileUrl,
          languageId: languageId,
  ...
  void removeUserFromTournament({required String userId}) {
    if (state is TournamentJoined) {
      //
      _tournamentRepository.removeUserFromTournament(
          uid: userId, tournament: (state as TournamentJoined).tournament);
    } else if (state is TournamentCreated) {
      //
      _tournamentRepository.removeUserFromTournament(
          uid: userId, tournament: (state as TournamentCreated).tournament);
    }
  }

  void updateTournamentBattlesResult(
      {required String tournamentBattleId, required String winnerId}) {
    if (state is TournamentStarted) {
      _tournamentRepository.updateTournamentBattlesResult(
          tournamentId: (state as TournamentStarted).tournament.id,
          winnerId: winnerId,
  ...
  String getSemiFinalBattleId() {
    ...
    if (state is TournamentStarted) {
      ...
      if (semiFinals.length == 1) {
        ...
        return semiFinals.first['user2'].toString().isEmpty
            ? semiFinals.first['id'].toString()
            : "";
      }
  ...
  int getUserIndex(String uid) {
    //
    if (state is TournamentStarted) {
      return (state as TournamentStarted)
          .tournament
          .players
  ...
  String getQuaterFinalBattleId(String uid) {
    if (state is TournamentStarted) {
      ...
      for (var quaterFinalDetails
          in (state as TournamentStarted).tournament.quaterFinals) {
